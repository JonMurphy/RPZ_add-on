...
RPZ can block websites via categories and examples include: fake websites, annoying pop-up ads, newly registered domains, DoH bypass sites, bad "host" services, maliscious top level domains (i.e., *.zip, *.mov), piracy, gambling, pornography, and more.  This is dependent on the RPZ provider and the catagories chosen.

This RPZ add-on enables the RPZ functionality by adding a couple lines in a configuration file.  The add-on simply adds a configuration file and adds three scripts (config, metrics and sleep) to make RPZ easier for the admin to use.
...
RPZ providers can be recommended by IPFire or the local admin can choose their own RPZ providers.
